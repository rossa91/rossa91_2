...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True, num_bits=8, num_bits_weight=8, mixed=True):
    ...
    def forward(self, input):
        ...
        if self.mixed:
          qweight = mixed_quantize(self.weight, qparams=weight_qparams)
        else:
...
class QLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_weight=8, mixed=True):
